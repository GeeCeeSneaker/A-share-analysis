"""Compile Golden CANDIDATE Dataset v2 (audit R4-A1.1).

Layout (append-only, audit P1-01/02):
    golden_cases_v1.jsonl / truth_manifest_v1.json  (v1 snapshot, untouched)
    golden_cases_v2.jsonl / truth_manifest_v2.json  (immutable snapshot)
    truth_manifest.json                             (ACTIVE pointer)

v2 changes vs v1:
- source_hash -> case_semantic_hash (includes case_type + artifact hash)
- source_artifact_hash field (empty while COMPILED; the REVIEW workflow
  must seal real external artifacts - P0-02/06)
- event_id / event_class on every case; distinct-event counts are
  honestly reported (P0-04): 2 distinct ST_CAP events, 10 distinct
  delisted securities - the event gate correctly BLOCKS production runs
  until the human review adds the missing real events.

Re-run: python scripts/golden/compile_v2.py (deterministic).
"""
...
TRUTH_VERSION = "v2-candidate-20260822"
OUT_DIR = Path("data/golden/provider/amazingdata")
COMPILED_BY = "ai-compile-v2"
...
def _case(
    ...
    doc = {
        ...
        "source_artifact_hash": "",  # sealed by the REVIEW workflow (P0-06)
        "truth_version": TRUTH_VERSION,
        "reviewed_by": COMPILED_BY,
...
def st_cases() -> list[dict]:
    cases: list[dict] = []
    # positive ST cap events (2 distinct events - honest count)
    ...
    # negative samples (NEVER count as ST transition events)
    blue_chips = (
...
def main() -> None:
    ...
    dataset_file = "golden_cases_v2.jsonl"
    ...
    manifest = {
        ...
        "candidate_status": (
            "Golden CANDIDATE dataset (audit section 13): all entries COMPILED; "
            "not a Verified Truth basis. Distinct-event coverage is honestly "
            f"incomplete (ST_CAP={events.get('ST_CAP', 0)}<50, DELIST="
            f"{events.get('DELIST', 0)}<20); the REVIEW workflow adds real "
            "distinct events and seals external artifacts."
        ),
    }
    # immutable per-version snapshot + ACTIVE pointer (append-only, P1-01)
    (OUT_DIR / "truth_manifest_v2.json").write_text(
        json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8", newline="\n"
    )
    (OUT_DIR / "truth_manifest.json").write_text(
        json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8", newline="\n"
    )
    print(f"v2 candidate: {len(cases)} cases; by type: {counts}")
    print(f"distinct events: { {k: v for k, v in events.items() if v >= 1} }")
