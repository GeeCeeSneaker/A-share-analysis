...
_GENERATED_PROFILE_ID_RE = re.compile(r"^(?:UNKNOWN|TRIAL_SIMULATION)_[0-9a-f]{12}$")
_FREEZABLE_PRODUCTION_CANDIDATE_ID_RE = re.compile(r"^UNKNOWN_[0-9a-f]{12}$")
...
def is_generated_scrubbed_profile_id(value: object) -> bool:
    """Return True only for an identity generated by AccountProfile."""

    if not isinstance(value, str) or value != value.strip():
        return False
    return bool(_GENERATED_PROFILE_ID_RE.fullmatch(value))


def is_freezable_production_candidate_id(value: object) -> bool:
    """Return True only for a generated non-trial production candidate."""

    if not isinstance(value, str) or value != value.strip():
        return False
    return bool(_FREEZABLE_PRODUCTION_CANDIDATE_ID_RE.fullmatch(value))


def is_scrubbed_profile_id(value: object) -> bool:
    """Backward-compatible alias for the generated scrubbed-id predicate."""

    return is_generated_scrubbed_profile_id(value)
...
def _valid_frozen_identity(identity: FrozenProductionIdentity) -> bool:
    ...
    if not is_freezable_production_candidate_id(profile_id):
        return False
...
def production_account_status(profile: AccountProfile) -> tuple[AccountKind, str]:
    ...
    if not is_generated_scrubbed_profile_id(profile.account_profile_id):
        return (AccountKind.UNKNOWN, "account profile identity is not generated and scrubbed")
    if not is_freezable_production_candidate_id(profile.account_profile_id):
        return (
            AccountKind.UNKNOWN,
            "account profile identity is not a freezable non-trial generated identity",
        )
