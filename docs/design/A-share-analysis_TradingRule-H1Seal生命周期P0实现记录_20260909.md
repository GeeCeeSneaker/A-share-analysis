...
| 单次快照 | candidate bytes 只从版本库读取一次；hash、解析、变换均使用同一 bytes 对象 | 不会用第二次读取的新 bytes 封入 reviewed |
...
| post-commit | 重新加载 ACTIVE 并重跑完整 review/evidence gate | 已提交但不一致时返回 `REVIEW_COMMIT_INCONSISTENT`，不伪装为成功 |
...
6. candidate snapshot 后被修改时仍只使用初始 snapshot；
7. parent 在 seal 前或 staging 后移动时阻断并清理；
8. 预存 single-writer lock 在候选读取前阻断；
9. staging gate 失败清理 raw/bundle/version；
10. ACTIVE manifest 写入失败回滚并释放锁；
11. happy path post-commit bundle gate 为空且候选/旧 parent bytes 不变。

本地 focused 结果：`12 passed, 1 skipped`（13 个用例）；skip 仅表示当前 Windows 环境无法创建测试符号链接，不代表生产路径放宽该检查。
