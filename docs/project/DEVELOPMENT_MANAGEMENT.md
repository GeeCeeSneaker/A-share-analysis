...
- 只为 research_security_daily 定义 2020-01-01 至 2026-06-30 的历史物化合同；research_index_daily 继续 DISABLED_UNVERIFIED_INDEX_IDENTITY。
- 固化 verified CR-4 ReadModel 单 snapshot 输入、PIT/identity lineage、ReadModel 完整性验证先于筛选、Development / Validation A / Holdout 三分区和 78 个日期逻辑月份。
...
**Date**：2026-09-12
**Status**：`R1 ACCEPTED ON MAIN / CONSUMPTION SLICE MERGED / EXACT-HEAD REVIEW PASS / REQUIRED CI GREEN`
...
- PR #51 exact head `f0f64da0822d1f016ffe77be198e2ffe126852a0` 获独立审阅 `5189671642` 的 PASS / MERGE AUTHORIZED，required CI #567 三个平台均成功，并以 merge commit `53257c36e8ada5576f5d2dfce0947710ee24694c` 合并到 main。index panel、CR-5 feature join、R2 market-state 语义、BSE mapping、第三次 Formal、Golden/H1/global baseline 和策略工作仍不在授权范围内。
...
- 旧 `v20260909-h1-compiled/rules.yaml` 恢复为 `main@a797f1186209e7548167e2fa652ce78715f24180` 原始字节，manifest-style hash `75d21777f1f135c47b963868641dfffc5428c5c5d897e17480a91ebaec1edd51`；未修改旧目录。
...
- PR：#20；merge commit：`b03ada73bc15d5e1ea1a62a28962ba8204b5bd51`
- Reviewer 最终核验 head：`4fb60f1cbb5a23f71bcbbfffbc72d488fd2a157d`
- pre-merge main：`669759adf34bece4c9e41c7be2ce2e9f858e5277`
- GitHub test-merge：`11cf4e4b811b0a5c90042087a544216863b4e1e1`
- Reviewer closure：`5130860878`
- final CI：run `34109324402` / #358；Windows py3.14、Windows py3.12、Ubuntu py3.14 全部 SUCCESS
- 本次合并后记录提交前的 main HEAD：`06cdab7b6985effd26bdf2309eeff0f154fff366`

**执行结论**

- v5 `v5-candidate-20260907` 保持 125 条 `COMPILED`；113 条 carry-forward verified，12 条 corrected cases 是唯一新增人工工作范围。
- GT-H3R/GT-H3R.1 的候选重建、复合来源契约、审计历史恢复和 CI 门禁已按 Reviewer closure 冻结，不在人工复审阶段重新设计。
- 当前 main 的 12-case Markdown/Excel 结果栏、反馈、审阅人和日期仍为空；本条记录不代表任何人工 APPROVE/REJECT，也不产生 `REVIEWED` 或 seal。
- 真实 Human Reviewer 仍需逐条打开全部 required official sources，并在受审计记录中提交 12/12 结果、113 条沿用确认、legacy `50` 的中和/定义和最终 human marker。
- 在上述声明被核验前，GT-H3B、`review.py`、Production B1-B7、Data Sufficiency、Provider capability verdict、2020+ backfill、策略/回测/交易均保持 BLOCKED。
...
## DM-20260905-147 · Post-merge T1 controlled online bootstrap preflight

**Type**：C1 — controlled production identity preflight  
**Date**：2026-09-05  
**Status**：NOT_TESTABLE_ACCOUNT / BLOCKED_BY_LOCAL_SECRET_INJECTION / PENDING_REVIEW  
**Evidence**：main head `c22a7111cda3ba9c86ca17aec4cfd85a2ee1955a` 包含 PR #9 merge commit `f38e77ff2cbcf040837bc1c15504f847e1cfb1d8`；Windows Python 3.14.6 离线 preflight 实际加载 `AmazingData==1.1.9` 与 `tgw==1.0.9.2`，状态 `OFFLINE_RUNTIME_VERIFIED`。

- 受控 online bootstrap 未进入登录：本地工作区没有 `.env`，进程环境没有 `TGW_*` 变量；安全无凭证检查返回 `NOT_TESTABLE_ACCOUNT` / exit 2。该结果不是 live bootstrap，不产生 candidate，也不构成账号事实。
- `configs/production_account.yaml` 继续为空；凭证、Token、真实 endpoint、raw profile 和 raw SDK 输出未进入 GitHub。
- T1 未完成前不进入 T2 human confirmation、T3 identity-freeze、Production B1-B7、Data Sufficiency、verdict 或 Provider approval。

**Next**

- 由受控 Windows operator 在本地进程环境或未被 Git 跟踪的 `.env` 中安全注入凭证后，重新执行仓库唯一 bootstrap 入口；先审查 scrubbed projection，再决定是否进入 T2。
...
**Type**：C1 — controlled production identity bootstrap verification  
**Date**：2026-09-04  
**Status**：VERIFIED (CI) / READY_FOR_CONTROLLED_RUN / PENDING_REVIEW  
**Trigger**：bootstrap formatter/status corrections completed and run 258 finished.

- run `33881832744`（run `258`）在 Ubuntu 3.14、Windows 3.12、Windows 3.14 三矩阵全部成功；每腿 `1425 passed`，Ruff lint/format、mypy、Spike、SDK-absent 及适用的治理门禁均通过。
- 本次只验证 scrubbed bootstrap 的可执行边界；identity freeze、真实 Production B1-B7、Golden/Data Sufficiency Matrix、verdict 和 Provider approval 仍 pending。
- `configs/production_account.yaml` 保持空 profile；凭证、Token、host/port/raw profile 不进入仓库；不涉及 migration 或 CR-5/CR-6/2020+ changes。

**Evidence / Next**

- 在受控环境中仅通过环境/.env 注入凭证运行 bootstrap，记录 scrubbed candidate；Owner/Reviewer 人工确认前不得提交 production allowlist，确认后仍需按单一 production run 执行 B1-B7。

## DM-20260904-124 · P0-AD-01 formatter follow-up

**Type**：C0 — bootstrap formatter follow-up  
**Date**：2026-09-04  
**Status**：IN_PROGRESS / CI_PENDING / PENDING_REVIEW  
**Trigger**：run 257 的 Ruff lint 已通过，format check 要求缺少凭证分支的 `_safe_report` 调用恢复单行。

- 按 formatter 输出修正调用换行；保持 `NOT_TESTABLE_ACCOUNT` 优先级、doctor 不调用和脱敏边界不变。
- 本次不涉及账号、Token、host/port/raw profile、migration、CR-5/CR-6、2020+ history 或 Provider approval。

**Evidence / Next**

- run 257 三矩阵均在 format 阶段停止，尚未进入功能测试；等待新的完整 CI。

## DM-20260904-123 · P0-AD-01 missing-input classification correction

**Type**：C0 — bootstrap fail-closed status correction  
**Date**：2026-09-04  
**Status**：IN_PROGRESS / CI_PENDING / PENDING_REVIEW  
**Trigger**：run 256 的新增 bootstrap test 发现缺少凭证时 `NOT_TESTABLE_SDK` 覆盖了更直接的 `NOT_TESTABLE_ACCOUNT` 状态。

- 为安全报告增加 credentials-available 语义，缺少完整环境时优先返回 `NOT_TESTABLE_ACCOUNT`，并保持不调用 provider doctor。
- 本次不改变 SDK/runtime 检查、脱敏输出、人工确认、production allowlist、账号、Token、host/port/raw profile 或正式 B1-B7 语义。

**Evidence / Next**

- run 256 Ubuntu 3.14 在新增测试处失败，静态检查通过；等待修正后的三平台完整 CI。

## DM-20260904-122 · P0-AD-01 formatter correction

**Type**：C0 — bootstrap formatter correction  
**Date**：2026-09-04  
**Status**：IN_PROGRESS / CI_PENDING / PENDING_REVIEW  
**Trigger**：run 255 的 Ruff lint 已通过，format check 要求规范化 bootstrap 退出码条件表达式。

- 按 formatter 确定输出调整条件表达式换行；运行语义、脱敏字段、凭证边界和人工确认步骤不变。
- 本次不涉及账号、Token、host/port/raw profile、migration、CR-5/CR-6、2020+ history 或 Provider approval。

**Evidence / Next**

- run 255 三矩阵均在 format 阶段停止，尚未进入功能测试；等待新的完整 CI。
...
**Type**：C1 — formal CLI contract and recovery verification  
**Date**：2026-09-04  
**Status**：VERIFIED (CI) / PENDING_REVIEW  
**Trigger**：PR8.1 format correction 后的 run 253 完成三平台全量 CI 与治理门禁。

- Ubuntu 3.14、Windows 3.12、Windows 3.14 全部成功；每腿 `1422 passed`，Ruff lint/format、mypy、Spike、SDK-absent、DEVLOG 和 Management gates 均通过。
- CLI mode conflict、Production replay-all/fresh catalog、CLOSED 与 semantic FAIL 的 focused evidence 已通过 CI；任务书 Exit gate 已勾选，等待人工 Reviewer 复审。
- 该证据不覆盖正式账号 identity/entitlement、真实 Production B1-B7、Golden/Data Sufficiency Matrix、verdict 或 Provider approval；production profile 仍为空，PR #8 不自动合并。
- 本次不涉及账号、凭证、Token、host/port/raw profile、migration、CR-5/CR-6 或 2020+ history contract。

## DM-20260904-119 · PR8.1 format correction

**Type**：C0 — CI format blocker correction  
**Date**：2026-09-04  
**Status**：IN_PROGRESS / CI_PENDING / PENDING_REVIEW  
**Trigger**：run 252 的 Ruff check 已通过，format check 要求规范化 `spike_runner.py` 和 focused CLI test。

- 按 CI formatter 的确定输出修正函数签名、长字符串、测试 argv 设置和多余空行；不改变运行语义。
- 本次不涉及账号、凭证、Token、host/port/raw profile、migration、CR-5/CR-6、2020+ history、production identity 或 Provider approval。

**Evidence / Next**

- run 252 三矩阵均在 format check 阶段停止，尚未进入功能测试；本提交等待新的完整 CI 与治理 gates。

## DM-20260904-118 · PR8.1 CLI mode and replay-all recovery

**Type**：C1 — formal CLI contract honesty and recovery semantics  
**Date**：2026-09-04  
**Status**：IN_PROGRESS / CI_PENDING / PENDING_REVIEW  
**Trigger**：新增 PR8.1 复审要求发现四种 CLI mode 可静默优先级解释，Production resume 的 `--phase bN` 与实际持久化 catalog 能力不一致，且 runbook 对 semantic FAIL 的 FAILED 表述过强。

- 选择方案 A replay-all：Production resume 强制完整 B1-B7、禁止 caller phase selector、fresh unsealed catalog 不加载旧 partial catalog，成功后才覆盖 RUNNING run 的 catalog；旧 raw/anchor 保留。
- 增加 mode conflict、partial catalog rebuild/replay-all 和 semantic CLOSED + verdict NO_GO focused tests；runbook、PR8.1 requirement、DEVLOG 与管理总册同步。
- 本次不新增 migration，不修改 CR-5/CR-6/2020+ history/Provider capability，不写入账号、凭证、Token、host/port/raw profile。

**Evidence / Next**

- 等待三平台完整 CI、Ruff/format/mypy、Spike/SDK-absent 与文档治理门禁；CI 通过后再将 PR8.1 标记为 VERIFIED，PR #8 仍不自动合并。

## DM-20260904-117 · Formal Runner wiring CI verification

**Type**：C1 — formal Spike runner execution-boundary verification  
**Date**：2026-09-04  
**Status**：DONE / VERIFIED (CI) / PENDING_REVIEW  
**Trigger**：run 248 completed after the lint/format corrections and validated the full formal-runner wiring change.

- 三矩阵 Ubuntu 3.14、Windows 3.12、Windows 3.14 全部成功；每腿 `1414 passed`。
- Ruff lint/format、mypy、全量 pytest、Spike gates、SDK-absent、DEVLOG 和 Management gates 均通过。
- 该证据只确认仓库实现和离线/假数据测试的 contract closure；正式账号 identity/entitlement、真实 Production B1-B7、Golden/Data Sufficiency Matrix、verdict 和 Provider approval 仍独立 pending/block。
- 本次不涉及账号、凭证、Token、host/port/raw profile、migration、CR-5/CR-6 或 2020+ history contract。
...
**Date**：2026-09-04
**Status**：IN_PROGRESS / CI_PENDING / PENDING_REVIEW
**Trigger**：PR #8 复审发现 formal Production/Trial CLI 未传入 anchor connection、resume as-of 可漂移，且 context/setup failure 可能留下 RUNNING。

- Formal Production/Trial 现在使用 `load_config().paths.duckdb_path` 对应的持久 DuckDB，通过 `DuckDBConnectionManager(...).owner("read_write")` 获取单进程所有权，并在任何新 run 之前执行完整 migration chain 与 anchor-schema readiness check。
- 同一连接覆盖 formal run 的 ProbeContext、AnchoredRawEvidenceWriter、catalog flush 和终态写入；Production/Trial 不允许 `:memory:`，dry-run 的隔离内存连接保持不变。
- 新 run 需要显式合法 YYYYMMDD；resume 权威日期来自原 `SpikeRun.as_of_date`，显式日期不匹配拒绝；context/catalog/phase/setup failure 统一落到 FAILED/ABORTED/CLOSED 明确终态。
- 新增 focused tests 覆盖真实 CLI context factory、anchor 关闭后重开、无内存 formal DB、日期冻结/拒绝和 anchor failure fail-closed；CI 结果待当前提交验证。
- 本次没有修改历史 migration、CR-5/CR-6、Provider capability approval、production_account.yaml 或任何凭证/Token/host/port/raw profile。

**Evidence / Next**
- 本地/远程代码审查已确认 formal runner 的旧调用缺少 `conn`；当前提交等待 Ubuntu 3.14、Windows 3.12、Windows 3.14 以及全部治理 gates。
- CI 通过后再回填准确 commit/run；在人工确认 production identity/entitlement 前，不执行或宣称 formal Production B1-B7/verdict/capability approval。
...
- 修正 provider doctor / 安装手册对 `RUNTIME_PACKAGE_VERIFIED` 与 `RUNTIME_ACTUAL_LOAD_VERIFIED` 的区分；当前本地 native SDK smoke 已有独立记录，但 formal runner 仍未执行。
- 本次只改运行手册，不改变 CR-6 冻结语义、正式账号 frozen identity 规则或 Provider approval 状态。

## DM-20260904-111 · CR-6 合并后治理同步与 Provider 当前真相校正

**Status**：DONE / PENDING_REVIEW

- PR #6 已在 main 合并提交 `dda8c000d8585a95a66a91fbaa5072427053abb8` 合入；CR-6.0–6.4 与 ADR-026 的最终 Reviewer closure 已生效：**VERIFIED / CLOSED / FREEZE**。
...
- ADR-026、ADR-000 索引、CR-6 工作要求、Provider Verification 与本总册的当前状态已同步；历史试用账号、正式账号本地 SDK smoke、正式 production profile 未冻结三者已明确分开。
- 正式 Production B1-B7、Golden/Data Sufficiency Matrix、verdict 和 Provider capability approval 仍未完成；`configs/production_account.yaml` 继续为空。凭证与连接明文不入库、不入日志。
- 下一步是把受控 SDK 依赖环境与仓库源码环境接通，运行 provider-doctor 及单一 CLOSED PRODUCTION B1-B7；在人工确认 scrubbed identity/entitlement 前不得授予 capability APPROVED。

## DM-20260904-106 · 正式 SDK 冒烟完成，形式化 Production run 待执行

**Status**：LOCAL_SMOKE_PASS / FORMAL_RUN_PENDING / PENDING_REVIEW

- 受控本地 Python 3.14.6 环境已导入官方 `AmazingData==1.1.9`、`tgw==1.0.9.2` 与 `tables`；TGW runtime `V4.3.0.260626-rc2.0-YHZQ`，`uv pip check` 通过。
- 正式账号登录与 profile 解析成功；permission/function-permission 字段存在。SDK 原始 stdout/stderr 在边界内捕获，未将账号、Token、连接信息、原始 payload 或临时 profile 持久化。
...
## DM-20260904-109 · 历史边界测试路径表达式 formatter 修正
...
| CR-4 SnapshotBuilder + DuckDB ReadModel Rebuild | DONE | **VERIFIED / CLOSED / FREEZE** | CR-4 全链已完成，ADR-024 ACCEPTED，PR #1 已合并 |
| CR-5 Deterministic Feature Layer | DONE | **REOPENED** | 主体机制 PASS；仅剩 P1 bounded lineage，转入 CR-5.2 |
| CR-5.1 Registry Honest Execution + Feature Seal Closure | DONE | **VERIFIED / CLOSED / FREEZE** | Reviewer 已验证 correctness closure |
| CR-5.2 Bounded Selected-Input Lineage | IN_PROGRESS | **START / ACTIVE** | 10k sparse/member-bound、selected-input PIT、market O(N) guard 与治理同步 |
| CR-6 Market State Layer | BLOCKED | **BLOCKED_BY_CR-5.2** | CR-5.2 closure 后才可启动 |
...
## CR-5.1 Registry Honest Execution + Feature Seal Closure（VERIFIED / CLOSED / FREEZE）

Reviewer commit `67d37f8e51b086e0585ba12b53e529e49d70a427` formally verified
CR-5.1 correctness closure: Registry honest execution, Feature seal
cross-binding, numeric/finding semantics, active missingness and the original
1..66 mapping are frozen. The review reopened only the P1 bounded-lineage
complexity item as CR-5.2; CR-6 remains blocked and PR #2 remains unmerged.

Previous green evidence remains run `33759993886` (run 168), with Windows
3.12, Windows 3.14 and Ubuntu 3.14 all green. The current CR-5.2 code/test
head is `1bbfb2b9485fb62f8713e13584879fe33cb656fe`; its run `33766197492`
(run 171) is still queued / in progress at this document revision.

## CR-5.2 Bounded Selected-Input Lineage（CI VERIFIED / REVIEW PENDING）

CR-5.2 implementation and CI verification are complete on clean atomic history head `3e7a0c27c5c7ee058c05721fca2e7b837cc8bb8e` (PR #3). The final tree originated from `8281e258a7595f8e5fbbd8d0f7e023a494f0b821`; PR #2's history remains unchanged because its run 175 failure was corrected by a new branch rather than force-push or an expanded workflow exception.

Run `33814571568` (run 176) is green on Ubuntu 3.14, Windows 3.14, and Windows 3.12, with `1320 passed` on each leg. Ruff lint/format, mypy, Spike, SDK-absent, and the required Windows 3.14 DEVLOG/Management-doc gates all pass. The bounded selected-input lineage, Registry-derived member bound, bounded lag dependency, market set/order guard, and focused mutation/availability tests are covered by this matrix.

Numeric formulas, active finding truth, artifact schema, migration 023 and all State/score/signal/strategy/backtest/production work remain out of scope. CR-5.2 is DONE / PENDING_REVIEW pending Reviewer closure; PR #3 is not auto-merged, PR #2 is retained for historical traceability, and CR-6 remains BLOCKED_BY_CR-5.2.
...
## DM-20260904-083 — CR-5.2 atomic-history CI verification

**Type**：C4 bounded-lineage complexity closure / governance synchronization  
**Status**：DONE / PENDING_REVIEW  
**Trigger**：PR #2 run `33767742448`（run 175）正确暴露历史 `0fe9897` 代码提交未在同一 commit 更新 DEVLOG 的门禁问题；仓库 workflow 明确禁止扩展例外和 force-push 重写。  
**Resolution**：从 `main` 以 CR-5.2 最终树创建 clean branch `codex/cr-5-feature-layer-20260904`，原子提交 `3e7a0c27c5c7ee058c05721fca2e7b837cc8bb8e`，保持 PR #2 历史可追踪并以 PR #3 接受同一完整审计。  
**Evidence**：GitHub Actions run `33814571568`（run 176）三矩阵 success；Ubuntu 3.14、Windows 3.14、Windows 3.12 每腿 `1320 passed`；Ruff lint/formatter、mypy、full pytest、Spike、SDK-absent 全部通过；Windows 3.14 DEVLOG/Management-doc gates success。  
**Current State**：CR-5.2 implementation + CI complete；Reviewer closure pending；ADR-025 仍 PROPOSED；PR #3 OPEN / NOT MERGED，PR #2 保留但不作为合并目标；CR-6 继续 BLOCKED_BY_CR-5.2。  
**Reviewer**：PENDING_REVIEW

## DM-20260903-082 — CR-5.2 bounded selected-input lineage implementation

**Type**：C4 bounded-lineage complexity closure / governance synchronization  
**Status**：IN_PROGRESS / PENDING_REVIEW  
**Trigger**：Reviewer 文档 `A-share-analysis_CR-5.1复审与CR-5.2_BoundedLineageComplexity收口要求_20260903.md`（2026-09-03 21:42 +08:00）确认 CR-5.1 correctness VERIFIED / CLOSED / FREEZE，并将唯一剩余 P1 bounded-lineage complexity 置于 CR-5.2 START / ACTIVE；CR-6 继续 BLOCKED_BY_CR-5.2。  
**Implementation**：security row lineage 绑定 current observation、固定 observed/lag dependencies 与 selected valid amount/volatility members；`FeatureExecutionPlan.max_security_lineage_members` 从 Registry/plan 推导上界并由 engine enforcement；market verifier 使用 set + previous-date order guard；新增 10k sparse/member-bound、mutation/availability、duplicate/order 与 structural focused tests。  
**Scope**：不改 Feature numeric formulas、active finding truth、artifact schema 或 migration 023；不启动 CR-6；不触碰 CR-2/3/4/production。  
**Evidence**：代码/test head `1bbfb2b9485fb62f8713e13584879fe33cb656fe`；CI run `33766197492`（run 171）在本文修订时 queued / in progress，结果待实际返回；上一轮 CR-5.1 green evidence 为 run `33759993886`（run 168）。  
**Reviewer**：PENDING_REVIEW
## DM-20260903-081 — CR-5.1 CI verification

**Type**：C4 correctness closure verification / governance synchronization
**Status**：DONE / PENDING_REVIEW
**Evidence**：GitHub Actions run `33758109611`（run 167）对 CR-5.1 code head `06106c27652e14f13d360fd3e153ececb39a4434` 的 Windows 3.12、Windows 3.14、Ubuntu 3.14 三腿全部 success；每腿 `1312 passed`。Ruff lint/formatter、mypy、full pytest、Spike framework、SDK-absent 全部通过；Windows 3.14 的 DEVLOG gate 与 Management-doc gate 通过，其他两腿按 workflow 条件跳过治理 gates。
**Decision**：CR-5.1 implementation evidence complete；ADR-025 保持 PROPOSED / PENDING_REVIEW，等待 Reviewer closure。PR #2 保持 OPEN / NOT MERGED；CR-6 继续 BLOCKED_BY_CR-5.1。
**Commit**：本次为文档与证据同步；代码证据锚定 `06106c27652e14f13d360fd3e153ececb39a4434`。
**Reviewer**：PENDING_REVIEW
...
**Verification**：代码 head `06106c27652e14f13d360fd3e153ececb39a4434` 对应 GitHub Actions run `33758109611`（run 167）三矩阵 success；Windows 3.12、Windows 3.14、Ubuntu 3.14 每腿 `1312 passed`，Ruff lint/formatter、mypy、full pytest、Spike、SDK-absent 均通过；Windows 3.14 的 DEVLOG 与 Management-doc gates success，其他两腿按 workflow 条件跳过治理 gates。  
**Implementation Commit**：`06106c27652e14f13d360fd3e153ececb39a4434`（代码 head；本治理同步另行提交）。
...
## DM-CR-20260903-078 — CR-5 Deterministic Feature Layer + PIT Feature Snapshot

**Type**：C2（Feature / PIT / lineage / artifact contract）  
**Status**：DONE（Implementation）/ PENDING_REVIEW  
**Trigger**：CR-4.4 final review accepted CR-4 full-chain closure and issued the CR-5 work requirement after PR #1 merge.  
**Old Contract**：CR-4 produced verified Snapshot and ReadModel, but no governed feature input boundary, feature registry, PIT feature lineage, deterministic feature identity, feature findings artifact, or feature ledger.  
**New Contract**：
- `FeatureBuilder.build(snapshot_id, feature_set_id)` accepts only an explicit verified ReadModel world and a static registry set; no latest/best, fallback, direct Snapshot/Canonical/Raw/Provider access, or hidden rebuild;
- V1 emits only `UNADJUSTED_CANONICAL` raw-price features, `OBSERVED_SECURITY_BARS` windows, and `OBSERVED_DAILY_BAR_UNIVERSE` breadth; adjusted/total returns, session semantics, full-universe claims, State/strategy semantics remain blocked;
- null, unsafe denominator, insufficient history, optional omission, and non-finite outcomes are deterministic typed findings; no fill, sentinel, copied suspension value, shortened window, or silent drop;
- every row carries knowledge-time `feature_available_at` and ordered source lineage; one feature run binds one Snapshot world + one Registry/contract/code identity;
- exact feature artifacts are immutable and recoverable, manifest-last / ledger-last, and the public verifier replays `compute_feature_set` from the verified ReadModel and consumes all physical/semantic seals.
**Reason**：为未来 CR-6 State 提供可确定性重建、PIT 可追溯且缺失原因可审计的特征事实层，不提前引入策略判断。  
**Affected Modules**：`src/ashare_state/features/`、`migrations/023_feature_build.sql`、`tests/integration/test_features.py`  
**Affected Data**：`meta_feature_build`；`feature/contract=feature-v1/.../manifest.json` 及三类 Parquet correctness artifacts  
**Compatibility**：CR-2/CR-3/CR-4 frozen inputs unchanged；新增 023 migration tail；Feature artifacts require current Registry / builder fingerprint for consumption。  
**Migration / Backfill**：from-zero 与 022→023 upgrade tests；无历史 Feature backfill。  
**Tests**：CR-5 contract matrix added; GitHub Actions validation pending.  
**ADR**：[ADR-025](../adr/ADR-025_feature_layer_pit_missingness.md)；ADR-024 merged/accepted.  
**Commit**：this CR-5 implementation commit  
**Reviewer**：PENDING_REVIEW

## DM-CR-20260903-079 — CR-5 CI 完整验证与 Reviewer closure 前状态

**Type**：C2（Feature / PIT / lineage / artifact contract）  
**Status**：DONE（Implementation + CI）/ PENDING_REVIEW  
**Trigger**：PR #2 CR-5 implementation head 完成 CI 验证。  
**Evidence**：GitHub Actions run `33745226956`（run 155）三矩阵 success；Ubuntu 3.14、Windows 3.12、Windows 3.14 每腿 `1270 passed`，Ruff lint、formatter、mypy、Spike、SDK-absent 全部通过；Windows 3.14 的 DEVLOG 与 Management-doc gates success。  
**Current State**：Feature Layer implementation 与 CI 已完成；CR-5 仍等待 Reviewer closure，PR #2 不自动合并；CR-6 State 与 production P0-M-1B 继续 blocked。  
**Commit**：implementation head `eaebce48ad373d7302f208f2f7fe7ddd53bf6cfb`；本条为验证状态文档同步。  
**Reviewer**：PENDING_REVIEW
...
## DM-CR-20260904-084 — CR-6.0 Governance Bootstrap

**Type**：C2（State interpretation contract and downstream layer bootstrap）  
**Status**：IN_PROGRESS / PENDING_REVIEW  
**Trigger**：PR #3 merged to main and the CR-6 activation document authorized the downstream State layer to start.  
**Old Contract**：CR-6 was BLOCKED_BY_CR-5.2 while ADR-025 and the management headers still described the pre-merge review state.  
**New Contract**：CR-5 and ADR-025 are synchronized to VERIFIED / CLOSED / FREEZE and ACCEPTED; CR-6 is START / ACTIVE under the separate deterministic State contract. CR-6.0 introduces ADR-026 in PROPOSED / PENDING_REVIEW status and typed State registry/model/schema skeletons.  
**Reason**：The reviewer-defined merge gate is now satisfied. The State layer needs an explicit interpretation contract before any runtime rule, artifact, ledger, or verifier can be implemented.  
**Affected Modules**：docs/adr/ADR-000_adr_index.md、docs/adr/ADR-025_feature_layer_pit_missingness.md、docs/adr/ADR-026_deterministic_market_state_interpretation.md、docs/DEVLOG.md、docs/project/DEVELOPMENT_MANAGEMENT.md、src/ashare_state/state/  
**Compatibility**：CR-5 V1 formulas, windows, universe, missingness, lineage, migration 023, and artifacts remain frozen. No State runtime or migration is introduced in CR-6.0.  
**Tests / Evidence**：upstream run 33818320010 (run 179) SUCCESS; CR-6.0 is a documentation/type/schema bootstrap and will be covered by the existing static CI plus later State contract tests.  
**ADR**：[ADR-026](../adr/ADR-026_deterministic_market_state_interpretation.md)  
**Commit**：CR-6.0 bootstrap commit  
**Reviewer**：PENDING_REVIEW


---

## DM-CR-20260904-085 — CR-6.1 Registry and Deterministic State Engine

**Type**：C2（State interpretation contract implementation）  
**Status**：IN_PROGRESS / PENDING_REVIEW  
**Trigger**：CR-6.0 governance bootstrap completed on the merged CR-5 mainline.  
**Old Contract**：CR-6 had only typed identity/schema skeletons and no executable State declarations.  
**New Contract**：A single static State Registry declares exactly four V1 descriptive dimensions and compiles them into a typed plan with exact dependencies, enums, rule IDs, threshold policies, missingness, availability, eligibility, and one handler per supported declaration. The shared engine computes deterministic State rows and typed findings from one VerifiedFeatureRun market projection.  
**Reason**：Make Registry declarations the sole rule truth before adding publication or replay infrastructure.  
**Affected Modules**：src/ashare_state/state/registry.py、src/ashare_state/state/engine.py、src/ashare_state/state/schema.py、src/ashare_state/state/__init__.py、tests/integration/test_state.py、docs/DEVLOG.md  
**Compatibility**：CR-5 formulas, artifacts, migration 023, and public Feature verifier remain unchanged. CR-6.1 adds no StateBuilder, artifact, ledger, migration, prediction, or strategy behavior.  
**Tests / Evidence**：CR-6.1 focused tests map the Registry and rule semantics groups 11–44; full three-platform CI remains the acceptance evidence.  
**ADR**：[ADR-026](../adr/ADR-026_deterministic_market_state_interpretation.md)  
...
## Change Record: DM-CR-20260904-105

- **Type**：C1/C2 — formal provider validation status update
- **Date**：2026-09-04
- **Status**：IN_PROGRESS / BLOCKED_BY_OFFICIAL_SDK / PENDING_REVIEW
- **Trigger**：Owner supplied formal-account connection information for the pending Production Spike.
- **Evidence**：An independent TCP probe found both Owner-provided candidate service endpoints reachable on the configured port. The controlled Python 3.14.6 environment has no official `AmazingData` or `tgw` wheel, so no login request was sent and no account profile or B1-B7 evidence was produced.
...
## DM-20260908-POSTMERGE-003 · Real v7 sealed-corpus integration coverage

**Type**：C1 — offline integration hardening  
**Date**：2026-09-08  
**Status**：IMPLEMENTED / CI VALIDATION PENDING  
**Evidence**：tests/integration/test_gt_h3b_execution.py；PR branch `test/gt-h3b-real-seal-integration-20260908`。
...
## DM-20260912-CR7-R1-028 · R1 research panel v1 小样本实现

**Type**：C1 — CR-7 research-ready daily implementation slice
**Date**：2026-09-12
**Status**：`R1 SECURITY DAILY SMALL-FIXTURE IMPLEMENTED / LOCAL FOCUSED QA GREEN / KEEP DRAFT / INDEPENDENT REVIEW AND CI PENDING`
...
- `research_index_daily` 当前返回 `DISABLED_UNVERIFIED_INDEX_IDENTITY`，没有用代码前缀、未验证 index code 或 Provider 直连猜测指数身份；待 index semantics/path 和 versioned identity/read-model join 获得明确验证后再单独开任务。
- R1 不公开 CR-5 feature columns/feature run join；避免复制同名公式，feature join、market breadth 和 index state 留到 R2。
- 本轮没有历史大规模物化/backfill、Provider sweep、第三次 Formal、Production、Golden/H1/global baseline、source-policy/fallback 或 strategy work。

**验收与闸门**

- 本地 focused tests：`tests/unit/test_r1_research_panel.py` 与 `tests/integration/test_r1_research_panel_integration.py`；提交前还需执行仓库质量门禁。
- 合并前必须由独立 Reviewer 对 exact head 核验 contract、默认排除、语义 fail-closed、PIT/lineage、immutable replay 和 index disabled 声明，并等待 Ubuntu 3.14、Windows 3.12、Windows 3.14 required CI 全绿。
- 在 scheduler 明确扩大范围前保持 Draft；不将本 PR 解释为全历史研究数据已完成，也不因此启动正式账号测试或生产物化。
...
## DM-20260914-CR7-COMPLETENESS-035 · AmazingData 月度完整性语义与 bounded Stage A/B

**Type**：P0 — CR-7 bounded real-source semantic validation
**Date**：2026-09-14
**Status**：`IMPLEMENTED / STAGE-A FAIL-CLOSED / STAGE-B IN PROGRESS / NO AUTHORITATIVE RECEIPT / DRAFT PR / INDEPENDENT DELTA REVIEW REQUIRED`
**Issue**：#59 `P0: validate AmazingData month-completeness semantics and bounded full-scope receipts`

**精确基线**：实际开发 base 为 `main@67f37d7ef7a084d775d14dbd0d474e1604e96d25`；已确认
PR #58 merge `31515992021e34517de0b764dd1ebb7f9e7ef35b` 为祖先。当前工作只覆盖 Issue #59
明确授权的 `2024-01`、`2020-01`、`2026-01`，不构成 78 月 backfill 授权。

**Stage A 真实观察**：

- 通过 reviewed AmazingData SPIKE facade 完成 2024-01 的 calendar、月度历史代码表、
  22 个交易日 exact-session 历史代码表、全量历史状态和全量 daily-bar 观察；成功交换数
  为 26，原始证据只保留在本地 ignored raw 目录，GitHub 只保留脱敏报告。
- 新规则版本为 `amazingdata-month-completeness-rule-v1`，适用性版本为
  `amazingdata-hist-code-list-exact-session-v1`。精确日代码表是正向 applicability 观察；
  `IS_SUSP_SEC=1` 才可把适用 pair 排除出必需 bar 集合；未知状态、形状漂移和 scope/request
  不一致均 fail-closed。
- 重放统计：`SUSPENSION_NON_TRADING=132`、`NOT_APPLICABLE_SESSION=115`、
  `required_bar_pair_count=112053`、`missing_required_pair_count=0`；3 个 `None` 日线成员
  均对应整月已证明停牌，现作为显式零行观察，不误报为数据损失。状态响应仍有 1 个无列
  成员，导致 32 个适用 pair `UNRESOLVED`；22 个返回 pair 未被证明为必需，整体结果为
  `FAIL_CLOSED`。报告见 `docs/provider_verification/cr7_month_completeness_stage_a_20260914.json`。
- 一次重取在第 10 个 exact-session 请求返回 `ProviderPermissionError`；未覆盖既有完整
  本地观察，未把部分请求当作通过。该问题及完整重放的计数/哈希已写入脱敏报告。

**本轮实现**：

- `month_completeness.py` 提供版本化、可 receipt-bound replay 的 expected-bar evaluator；
  明确区分 suspension、session 不适用、API/shape mismatch、unexplained missing、
  unresolved 与 returned extra。非适用仅来自精确日历史代码表，不能由 bar 缺失倒推。
- AmazingData acquisition receipt v2 新增历史状态与 exact-session code-list exchanges，
  首次取数和 retained-capture replay 使用相同规则；`dict[str,dataframe|None]` 的显式 null
  只在没有必需 pair 时允许，必需证券为 null 时仍结构错误并 fail-closed。
- Stage B 采用固定月份的进程隔离 worker；单月原生 SDK 阻塞不会阻塞另一月，超时只表示
  `SDK_NATIVE_CALL_PROCESS_TIMEOUT`，不伪装成 provider 的硬查询预算。没有新增 provider
  信任、签名、第三方 attestation 或多源仲裁。

**验证与下一闸门**：

- 聚焦语义/历史物化/RawWriter guard 回归通过；Ruff、`mypy src`、compileall、`uv pip check`
  通过；全量 pytest 退出码 0，保留 3 个既有 Windows symlink 权限 skip。
- Stage B 尚待实际运行；两个代表月必须分别得到可重放 authoritative receipt/materializer
  proof 或 sanitized exact blocker。当前 source-selection closure 仍为
  `CLOSURE_DESIGN_ONLY_NOT_ACTIVE`，所以即使数值语义通过，本 PR 也不自行铸造 receipt 或
  进入 materializer。
- 账号、密码、IP、端口、Token、Cookie、专有 SDK/runtime、原始 provider payload 和本地
  ignored 依赖不进入 GitHub；不执行 78 月物化、Formal B1-B7、Production、resume/verdict、
  BSE/index、CR-5/R2、Golden/H1、baseline 或策略工作。提交后保持 Draft，等待 exact-head
  CI 与独立 delta review，项目管理者决定 PASS / REMEDIATE / BLOCKED。
## DM-20260914-CR7-COMPLETENESS-035 · AmazingData 月度完整性语义与 bounded Stage A/B

**Type**：C1 — CR-7 bounded real-source completeness semantics / fail-closed remediation
**Date**：2026-09-14
**Status**：`IMPLEMENTED / STAGE A+B FAIL-CLOSED / LOCAL QA GREEN / DRAFT PR / INDEPENDENT DELTA REVIEW REQUIRED`
...
- 新增 versioned `amazingdata-month-completeness-rule-v1` 与
  `amazingdata-hist-code-list-exact-session-v1`。精确日历史 code list 定义 session
  applicability，历史状态 `IS_SUSP_SEC=1` 定义合法 suspension/non-trading；只有可读且
  `IS_SUSP_SEC=0` 的适用 pair 进入必需 bar 集合。未知、畸形、范围漂移、缺状态和返回
  extra 均不被 heuristic 吞掉，并保持 fail-closed。
- 把状态、精确日适用性和全量 daily-bar 交换绑定进 `AmazingDataAcquisitionReceipt` v2
  的验证/重放路径；`None` daily member 仅在该证券没有任何必需 pair 时可解释为合法空观测，
  对仍有必需 pair 的 `None` 仍 fail-closed。移除不成立的月度 Universe 全交叉积前提，
  保留 exact-session applicability 与状态范围一致性检查。
- Stage A 固定为 `2024-01`；Stage B 只允许固定 `2020-01` Development、`2026-01`
  Holdout。Stage B 使用每月独立 Python worker 和 900 秒父进程 OS 边界，单月 SDK 卡住
  不会阻止另一月，也不会把跨月 evidence 混用。

**真实观察与结论**：

- Stage A 的完整本地 raw replay 包含 22 个交易日、5,106 个月度证券、22 个精确日 code
  list、状态和 daily bar，共 26 个成功交换。离线评估得到 132 个
  `SUSPENSION_NON_TRADING`、115 个 `NOT_APPLICABLE_SESSION`、0 个必需 bar 缺失，但有
  1 个无列状态成员、32 个状态未决 pair 和 22 个尚未被状态证明为必需的返回 pair，故
...
- Stage B 的脱敏报告为
  ...
  bar 缺失均为 0，但两个月均不能签发 receipt。
- 三个月仍没有 `AUTHORITATIVE_UPSTREAM` receipt、materializer proof 或历史物化输入；
  source-selection closure 仍为 `CLOSURE_DESIGN_ONLY_NOT_ACTIVE`。下一项应是最小化解决
  状态接口的可读 schema 与完整月内语义，再仅重跑 Issue #59 授权的三个代表月。不得用
  缺失即未变化、另一数据源或推测性多源仲裁掩盖未决状态。

**验证与治理边界**：

- focused completeness/materialization tests、full pytest、Ruff、mypy、compileall、
  `uv pip check`、JSON parse 和 `git diff --check` 均须以本轮 exact head 重新核对；
  CI 和独立 delta review 未完成前保持 Draft，不自行批准、Ready 或合并。
- 账号、密码、IP、端口、Token、Cookie、私有 endpoint、专有 SDK/runtime、本地 raw
