...
**Verification**：Local 1235/0；ruff check / ruff format / mypy 全绿（78 源文件）；CI 同款 `uv run pytest` 复验；GitHub Actions 推送后 API 正向确认（run id 待回填）  
**Commit**：`<本批提交后回填>`（CR-4 首批 implementation；SHA 由 docs 回填 commit 补记）  
