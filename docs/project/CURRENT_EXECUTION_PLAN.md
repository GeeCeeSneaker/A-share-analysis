...
- Local QA: the previously verified full pytest suite, Mypy (111 source files), and affected-file Ruff checks passed. Evidence commit f469fae CI #750 passed Ruff lint but failed Ruff format only on the migration runner and new identity test; Mypy/pytest were skipped. A formatting-only follow-up is prepared, with exact-head CI pending. Final Owner/PM acceptance remains required; keep PR #77 Draft and do not merge or activate production scope.
