...
Local QA passed: focused credential/T1/runner boundary tests; full pytest `1930 passed, 9 skipped`; Ruff lint/format; Mypy (112 source files); and `uv pip check` after offline install from retained local SDK wheels. Exact-head GitHub CI run #761 passed all required jobs on head `07be642d81598f9873cf2def4e97c82a08910bcc`; it included Ubuntu/Python 3.14 and Windows/Python 3.12 and 3.14. GitHub's PR checks are the live source of truth after any later head update; the latest head must be green before merge. Independent review is pending; Issue #84 is not complete until review acceptance. See `docs/project/ISSUE84_TGW_CREDENTIAL_STORE_20260924.md` for the compact secret-free acceptance record.

Next gate: obtain an independent review of PR #87 and await its decision. PR #87 is ready for review and is not merged; before any merge, verify that GitHub checks are green for its then-current head. Do not close #84 until review is accepted. Do not run Production B1-B7 or reacquire historical data. Issue #39 Production remains unauthorized pending independent review.
