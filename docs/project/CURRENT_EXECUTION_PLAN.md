...
- **#97 / PR #100 — 研究价格与收益语义：ACTIVE。** 核心 reference-price linked-chain 实现 PR 已打开；exact-head Windows/Ubuntu CI 与独立审阅是合并门槛。代表性真实日线 corporate-action 对照尚缺；合成测试不冒充真实数据验收。IPO/no-limit 和长停牌规则如缺少受治理输入，继续保持禁用并单独记阻塞。
- **#98 — tracked 日增量 runner：ACTIVE。** PR #102 已从当前 main 以 draft 打开；exact-head CI 通过与独立审阅是合并门槛。runner、CLI、五日 fake-provider 集成与归档/异地副本巡检均已实现；真实五日 Provider 验收未完成。归档恢复 smoke 与旧 GT-H3B workflow 清理作为核心 PR A 后的小型 follow-up，不得延迟 PR A。
...
2. **#98：**完成 PR #102 CI/审阅；随后追加有界归档恢复读取 smoke 与旧 workflow 清理。恢复已接受的 2026-06 日线 Snapshot 与配套证据后，完成之后连续五个交易日、60/00/30/688 VWAP 单位检查、同目标无 Provider 重跑及异地归档校验。
