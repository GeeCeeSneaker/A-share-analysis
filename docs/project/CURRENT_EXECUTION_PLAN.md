...
## Completed P0 — Issue #84 secure TGW credential persistence (2026-09-24)
...
## Current active P0 — Issue #90 status-key and request-routing semantics (2026-09-25 UTC)

> Status: **OPEN / READY FOR INDEPENDENT PASS REVIEW. No history acquisition is authorized.**

Issue [#90](https://github.com/GeeCeeSneaker/A-share-analysis/issues/90) remains the unique active P0. Scheduler gate correction [comment #5824435432](https://github.com/GeeCeeSneaker/A-share-analysis/issues/90#issuecomment-5824435432) supersedes the earlier requirement to obtain a semantic explanation for the unsupported 1990-start response shape. That shape remains unclassified and must stay fail-closed; it is explicitly unsupported for Canonical acquisition.

The authorized production-shape check was run once using the same eight SH/SZ symbols, `begin_date=20200101`, `end_date=20200131`, and `is_local=False`. Native response inspection occurred before Canonical adaptation. SDK 1.1.9 returned eight tables whose response keys exactly matched the eight requested exchange-qualified symbols: six tables had 16 rows each; `688500.SH` and `605499.SH` were empty and remain unresolved. Across 96 returned rows: missing `TRADE_DATE` = 0, missing `MARKET_CODE` = 0, invalid/out-of-month dates = 0, exchange-qualified identity conflicts = 0, and duplicate `(symbol,date)` keys = 0. One request attempt; no raw response or credentials were persisted.

The exact sanitized results and scope limits are recorded in `docs/provider_verification/issue90_status_key_semantic_closure.md`. This satisfies the technical month-shape gate and is submitted for independent PASS review; it does **not** prove completeness for other months/symbols or authorize the 78-month build. Until #90 receives independent PASS and a separate acquisition task is opened, do not start history acquisition, BSE activation, capability promotion, or Formal B1–B7.
...
**Current task:** Issue #85 runtime simplification. Do not start Issue #86 or new data-domain work until #85 passes its exact-head CI and scheduler acceptance gates. Issue #76 and Issue #84 are historical/completed; no provider or production scope is implied.
