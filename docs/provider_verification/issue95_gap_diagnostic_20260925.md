...
After the narrow memory change, Python 3.14.7 validation passed: **304** focused Canonical/snapshot/readmodel/status/output-quarantine tests, Ruff lint, Ruff format check, mypy across 112 source files, `compileall`, and `git diff --check`. The full repository suite was not run locally. GitHub Actions run #804 passed on Windows/Ubuntu Python 3.14 for the earlier PR head `047dea9d7538e2d12a3f78dce2e0c2d47335d914`. The first CI run on the memory-remediation head (#807) failed at mypy with two typing/API errors and skipped pytest. A follow-up patch restores the consumption verifier's selected-row entry point while leaving ordinary Canonical verification columnar, and makes temporary-file ownership explicit. Local mypy and the 304-test regression set now pass; exact-head CI for that follow-up push is pending.
