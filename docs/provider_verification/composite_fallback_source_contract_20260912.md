...
- SSE、SZSE、BSE 的公开法律 / 数据声明对自动复制、下载、存储、抓取、商业使用
  作出不同程度的限制或要求，不能直接假定本项目已有生产化许可。
...
- 自动保存一手页面 / PDF、解析和生成衍生数据是否获得项目所需的许可 / 使用授权。
...
1. 项目管理者先对 SSE、SZSE、BSE 的自动读取、留存、解析和派生使用作出明确的
   许可 / 合规决定；没有这个决定，不激活 fallback。
