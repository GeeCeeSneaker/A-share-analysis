...
| Date arguments | Facade accepts integer `start_date`/`end_date` and forwards them as SDK `begin_date`/`end_date`. | Use inclusive YYYYMMDD bounds consistent with the existing facade; reject invalid/out-of-order bounds in the caller. |
