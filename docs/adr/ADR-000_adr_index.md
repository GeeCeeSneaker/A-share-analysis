...
| ADR-023 | AvailabilityPolicy + Canonical Source Selection（时间可用性策略与 Canonical 源选择） | PROPOSED | 2026-09-01 | [ADR-023](ADR-023_availability_policy_canonical_selection.md)，CR-3（DM-20260901-068：closure-verified 唯一输入 / identity bridge fail closed / OBSERVED_AT_INGEST as_of 先行过滤 / 静态版本化 SourcePolicy + EXACT reconciliation / immutable canonical artifacts + deterministic run identity / migration 018）；复审裁决 pending |
