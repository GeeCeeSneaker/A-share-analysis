...
| ADR-024 | SnapshotBuilder + DuckDB ReadModel（CR-4 快照与读模型层） | PROPOSED | 2026-09-03 | [ADR-024](ADR-024_snapshot_builder_readmodel.md)，CR-4（DM-20260903-075）：CR-4.1 canonical 公共消费验证器 / CR-4.2 SnapshotBuilder（版本化 schema registry + 确定性 identity + immutable artifacts + migration 022）/ CR-4.3 DuckDB ReadModel（temp→logical seal→原子替换）；含 CR-3 latent 缺陷（multi-domain semantic seal）显式申报修复；复审裁决 pending |
